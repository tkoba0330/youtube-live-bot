...
- **コンテキスト量の試算**（コメント生成1回あたりのLLM入力）:

  | 項目 | 30分配信 | 1時間 | 2時間 |
  |---|---|---|---|
  | 文字起こし | ~6,000 tok | ~12,000 tok | ~24,000 tok |
  | 画像解析結果（30秒間隔 × ~200tok/回） | ~12,000 tok | ~24,000 tok | ~48,000 tok |
  | システムプロンプト+キャラ設定 | ~1,000 tok | ~1,000 tok | ~1,000 tok |
  | **合計** | **~19,000 tok** | **~37,000 tok** | **~73,000 tok** |

  > 画像解析結果が文字起こしの約2倍のトークンを消費する点に注意。

- **コスト試算**（Claude Sonnet 4.6でコメント生成、90秒間隔）:

  | シナリオ | 呼び出し回数 | コンテキスト/回 | コスト/配信 |
  |---|---|---|---|
  | 30分・圧縮なし | 20回 | 19K tok | **$1.14** |
  | 30分・画像重複排除あり | 20回 | 10K tok | **$0.60** |
  | 2時間・圧縮なし | 80回 | 73K tok | **$17.52** |
  | 2時間・直近10分に圧縮 | 80回 | 12K tok | **$2.88** |

  別途、画像解析API（Gemini 2.5 Flash等）のコスト: 30分配信で約$0.005（無視できる水準）

- **レイテンシ**: Claude Sonnet 4.6の応答は通常1〜3秒。コメント投稿間隔（90秒）に対して十分高速
- **対策**:
  - 画像解析結果の重複排除を初期実装から組み込む（コスト半減効果）
  - API障害時のフォールバック（別プロバイダへの切り替え）を実装
...
#### 6. コンテキスト蓄積とコスト管理（影響度: 中）

- **課題**: 画像解析結果（30秒間隔 × ~200tok）が文字起こしの約2倍のトークンを消費し、コメント生成のたびに全コンテキストをLLMに送信するためコストが累積する
- **対策（段階的に実装）**:

  **Phase 1: 画像解析の重複排除（初期実装から必須）**
  - 同一スライドが表示されている間の解析結果をマージ（前回との類似度判定）
  - スライド切替を検知し、新規スライドのみ完全な解析結果を保持
  - 効果: 30分配信で画像解析トークンを約50%削減（12K → 6K tok）

  **Phase 2: スライディングウィンドウ（1時間超の配信対応時）**
  - 直近N分の生データ + それ以前の要約を保持
  - 文字起こし・画像解析それぞれに独立したウィンドウを設定

  **Phase 3: 定期要約（2時間超の長時間配信対応時）**
  - 一定間隔でLLMによるコンテキスト要約を生成
  - 古いデータを要約で置き換え、コンテキストサイズを一定に維持

- **ContextManagerの設計方針**:
  - `compress()` インターフェースを初期から定義
  - Phase 1（重複排除）は初期実装に含める
  - Phase 2-3は将来の拡張ポイントとして設計のみ
...
| LLM APIコスト/レイテンシ | 中 | 中（画像解析結果がコスト主因） | 高 |
| コンテキスト蓄積・圧縮 | 中 | Phase 1: 低 / Phase 2-3: 中 | 高（Phase 1は初期実装） |
